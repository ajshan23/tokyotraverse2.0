...
const ProductInner = ({ product }) => {
  ...
  const handleLike = async () => {
    if (liked) {
      await axios
        .post(
          `http://localhost:5000/api/v1/users/removefromwhishlist/${product._id}`
        )
        .then((response) => {
      ...
      await axios
        .post(
          `http://localhost:5000/api/v1/users/addtowhishlist/${product._id}`
        )
        .then((response) => {
          if (response.data.success === true) {
            setLiked(true);
  ...
  const likedOrNot = async () => {
    {
      await axios
        .post(
          `http://localhost:5000/api/v1/users/checkinwhishlist/${product._id}`
        )
        .then((response) => {
          if (response.data.success === true) {
            setLiked(true);
        ...
        <div className="w-full md:w-96 h-full flex flex-col md:border-r-2 border-red-600">
          ...
          <div className="w-full h-full flex flex-row justify-center gap-4">
            <img
              src={product?.image}
              alt=""
              className="w-20 h-20 border-x md:border-x-2 border-red-600"
            />
            <img
              src={product?.image}
              alt=""
        ...
        <div className="w-full h-full md:pl-10 flex flex-col justify-evenly gap-3 items-start ">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-row gap-2 items-center text-xl">
                <button
                  ...
                  className="px-6 py-2 rounded-sm text-red-600 border-2 border-red-600 active:bg-red-600 active:text-white cursor-pointer transition-all ease-in-out"
                >
                  +
                </button>
                <div>{quantity}</div>
          ...
          <div className="flex flex-col items-start">
            <div className="flex items-center justify-center gap-3">
              <div className="font-lexend font-bold text-3xl">
                {" "}
                ${product?.price}
              </div>
            ...
            {product?.stock < 5 && (
              <div className="text-sm text-red-600">
                {product.stock===0?<div className="text-xl">Out of stocks</div>:
                <div>
